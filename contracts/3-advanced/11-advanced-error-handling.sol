...
contract LearnErrorHandling {
    ...
    function solarCalc() public {
        require(sunny, "It's not sunny outside");
        finalCalc += 3;
        assert(finalCalc !=6);
    }

    // finalCalc can never equeal 6 
...
contract Vendor{
    ...
    modifier onlySeller() {
        require(msg.sender == seller, "Only Sellers can use this function");
        _;
    }
    function becomeSeller() public {
        // we check to see if the msg sender is the seller
        seller = msg.sender;
    }
